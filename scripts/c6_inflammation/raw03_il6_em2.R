...
info <- read.csv("/scratch/users/k1787261/cytokines/LSinfo/il6_rerun_info_191id_LS.csv")
...
info$il6_log <- as.numeric(info$il6_log)
...
#model 1:
f1 <- y ~ il6_log + Age + CD8T + CD4T + NK + Bcell + Mono + Gran + (1|Sentrix_ID) + (1|Zygosity) + (1|KCLfam) + Date_diff + crp_mgl

f2 <- y ~ Age + CD8T + CD4T + NK + Bcell + Mono + Gran + (1|Sentrix_ID) + (1|Zygosity) + (1|KCLfam) + Date_diff + crp_mgl
...
write.csv(resu_out, "/scratch/users/k1787261/cytokines/LSresults/raw/revised2_il6_extended2_twinsuk_20220622.csv", quote = F, row.names = F)
