...
info <- read.csv("/scratch/users/k1787261/cytokines/LSinfo/leptin_rerun_info_97id_LS.csv")
...
info$leptin_log <- as.numeric(info$leptin_log)
...
#model 1:
f1 <- y ~ leptin_log + Age + BMI_Nearest + CD8T + CD4T + NK + Bcell + Mono + Gran + (1|Sentrix_ID) + (1|Zygosity) + (1|KCLfam) + Date_diff
...
write.csv(resu_out, "/scratch/users/k1787261/cytokines/LSresults/raw/revised2_leptin_extended2_twinsuk_20220622.csv", quote = F, row.names = F)
