...
info$TGlog <- as.numeric(info$TGlog)
...
f1 <- y ~ TGlog + sex + BMI + smok + CD8T + CD4T + NK + Bcell + Mono + Gran + (1|Sentrix_ID) + (1|Sentrix_Position) + medchol_53x
...
write.csv(resu_out, "/scratch/users/k1787261/rory_lipids/results/qq/qq104_NSHD99_TGlog_medco_770id.csv", quote = F, row.names = F)
